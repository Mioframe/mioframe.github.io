import { r as q, y as w, l as D, z as ce, K as Y, p as J, a4 as Z, a5 as be, a0 as Q, a6 as fe, a7 as ge, X as we, Z as Se, _ as Ee, L as Te, s as _, a1 as Oe, m as _e, G as ue, b as N, u as Ae } from "./iframe-BmKBn1Mu.js";
function Re(e, t, n = {}) {
  let o, u, i, l = true;
  const a = () => {
    l = true, i();
...
const Ce = (e) => e != null, De = Object.prototype.toString, Me = (e) => De.call(e) === "[object Object]", L = () => {
};
function Ne(...e) {
  if (e.length !== 1) return ge(...e);
  const t = e[0];
  return typeof t == "function" ? we(Z(() => ({ get: t, set: L }))) : q(t);
}
function ee(e, t) {
  function n(...o) {
...
function Le(e, t = 200, n = false, o = true, u = false) {
...
function K(e, t, n = {}) {
  ...
  return { stop: ze(e, t, { ...i, eventFilter: l }), pause: a, resume: f, isActive: c };
}
function nt(e, t, ...[n]) {
...
function Ge(e, t, n, o = {}) {
  ...
  const { flush: i = "pre", deep: l = true, listenToStorageChanges: a = true, writeDefaults: f = true, mergeDefaults: c = false, shallow: s, window: r = R, eventFilter: v, onError: d = (m) => {
    console.error(m);
  }, initOnMounted: y } = o, S = (s ? _ : q)(typeof t == "function" ? t() : t), h = N(() => w(e));
  if (!n) try {
  ...
  if (!n) return S;
  const g = w(t), T = Ie(g), E = (u = o.serializer) !== null && u !== void 0 ? u : Je[T], { pause: C, resume: z } = K(S, (m) => P(m), { flush: i, deep: l, eventFilter: v });
  D(h, () => F(), { flush: i });
  let H = false;
  const j = (m) => {
    y && !H || F(m);
  ...
  r && a && (n instanceof Storage ? x(r, "storage", j, { passive: true }) : x(r, se, U)), y ? V(() => {
    H = true, F();
  }) : F();
  function p(m, b) {
    if (r) {
...
function Ke(e) {
  const t = Q(), n = Re(() => null, () => t.proxy.$el);
...
const ae = 1;
function dt(e, t = {}) {
  const { throttle: n = 0, idle: o = 200, onStop: u = L, onScroll: i = L, offset: l = { left: 0, right: 0, top: 0, bottom: 0 }, observe: a = { mutation: false }, eventListenerOptions: f = { capture: false, passive: true }, behavior: c = "auto", window: s = R, onError: r = (p) => {
    console.error(p);
  ...
  const T = _(false), E = ue({ left: true, right: false, top: true, bottom: false }), C = ue({ left: false, right: false, top: false, bottom: false }), z = (p) => {
    ...
    let B = M.scrollTop;
    p === s.document && !B && (B = s.document.body.scrollTop), C.top = B < y.value, C.bottom = B > y.value;
    const re = Math.abs(B) <= (l.top || 0), ie = Math.abs(B) + M.clientHeight >= M.scrollHeight - (l.bottom || 0) - ae;
    F === "flex" && W === "column-reverse" ? (E.top = ie, E.bottom = re) : (E.top = re, E.bottom = ie), y.value = B;
  }, U = (p) => {
    var P;
    s && (j((P = p.target.documentElement) !== null && P !== void 0 ? P : p.target), T.value = true, H(p), i(p));
  };
  return x(e, "scroll", n ? Le(U, n, true, false) : U, f), V(() => {
    try {
      const p = w(e);
      if (!p) return;
      j(p);
    ...
    const p = w(e);
    p && j(p);
  }, { attributes: true, childList: true, subtree: true }), x(e, "scrollend", z, f), { x: S, y: h, isScrolling: T, arrivedState: E, directions: C, measure() {
    const p = w(e);
    s && p && j(p);
  } };
}
