import { h as u, q as A, a0 as R, H as k, D } from "./iframe-DdvTzK_t.js";
const v = (e) => {
};
...
}, F = (e, t) => e === t ? 0 : e.compareDocumentPosition(t) & Node.DOCUMENT_POSITION_FOLLOWING ? -1 : 1, j = (e) => {
...
}, O = (e) => e.getItems().map(j).filter((t) => t !== null).sort((t, n) => F(t.primaryElement, n.primaryElement)), T = ".md-list-item__primary-action, .md-list-item__trailing-action", q = (e, t) => {
...
_.__docgenInfo = Object.assign({ displayName: _.name ?? _.__name }, { exportName: "useListActionKeyboard", displayName: "useListActionKeyboard", description: "Wires Material List keyboard navigation for non-selection `MDList` containers holding\n`single-action` / `multi-action` rows: `ArrowDown`/`ArrowUp` move vertically within the\nsame action column, `ArrowLeft`/`ArrowRight` move between a row's primary and trailing\naction, and `Home`/`End` move to the first/last enabled row in the current column.\n`Space`/`Enter` activation is handled by the focused native button/link itself.", type: 2, props: [{ name: "value", global: false, description: "", tags: [], required: true, type: "HTMLElement | null", schema: "HTMLElement | null", declarations: [] }, { name: "__@RefSymbol@300", global: false, description: `Type differentiator only.
We need this to be in public d.ts but don't want it to show up in IDE
...
E.__docgenInfo = Object.assign({ displayName: E.name ?? E.__name }, { exportName: "useListSelectionKeyboard", displayName: "useListSelectionKeyboard", description: "Wires roving keyboard navigation and tab-stop synchronization for a listbox\ncontainer. Only active when `enabled` is true (i.e. when selectionMode !== 'none').", type: 2, props: [{ name: "value", global: false, description: "", tags: [], required: true, type: "HTMLElement | null", schema: "HTMLElement | null", declarations: [] }, { name: "__@RefSymbol@300", global: false, description: `Type differentiator only.
We need this to be in public d.ts but don't want it to show up in IDE
